...
const { CREATE_PROJECT, CL_TIP, CREATE_PROJECT_LOADING, DIR_IS_EXITS, START_INSTALL, PACKAGE_MANAGER_CONFIG } = require('../constants/index.js');
const { isDirExits } = require('../utils/dir');
const { taskSuccessTip } = require('../utils/tip');
const { commandSpawn } = require('../utils/terminal');
const { dirIsExitsOrCreate } = require('../utils/dir');
...
const installAndRunProject = async (projectName, packageManager) => {
  dirIsExitsOrCreate(PACKAGE_MANAGER_CONFIG, packageManager);
  await commandSpawn('git', ['init'], { cwd: `./${projectName}` });
 ...
 * @returns
 */
const createProjectActions = async (projectName, isRunProject, isCustorm, packageManager) => {
  const baseConfig = {
